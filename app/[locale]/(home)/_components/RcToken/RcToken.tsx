...
export async function RcToken({ t }: Props) {
  ...
  const { biodiversity, inspections, trees } =
    await inspectionService.getImpactPerEra({ era: 1 });

  return (
    <section className="flex flex-col py-10 lg:py-20">
      <h3 className="text-xl font-bold uppercase text-center md:text-3xl">
      ...
      <div className="flex flex-col lg:flex-row lg:mt-10">
        <div className="flex flex-wrap items-center justify-center gap-8 mt-5 w-full lg:justify-start lg:max-w-[50%] lg:border-r lg:py-5">
          ...
          />
          <CardTokenInfo label={t("totalCertified")} value={totalCertified} />
          <CardTokenInfo label={t("trees")} value={trees} />
          <CardTokenInfo label={t("biodiversity")} value={biodiversity} />
          <CardTokenInfo label={t("inspections")} value={inspections} />
        </div>
